...
try {
  if (isCompiled) {
    logger.notice("Running as compiled executable, loading ID generator");

    // Try nanoid v3 first (CommonJS compatible)
    try {
      const { nanoid: nanoidFunction } = require("nanoid");
      nanoid = nanoidFunction;
      logger.notice("Successfully loaded nanoid v3 using CommonJS");
    } catch (nanoidError) {
      logger.warning("Nanoid failed, trying uuid", {
      ...
      try {
        ...
        logger.warning("Successfully loaded uuid as nanoid fallback");
      } catch (uuidError) {
    ...
    logger.notice("Loading ID generator in normal Node.js environment");

    try {
      const { nanoid: nanoidFunction } = require("nanoid");
      nanoid = nanoidFunction;
      logger.notice("Successfully loaded nanoid v3");
    } catch (nanoidError) {
      logger.warning("Nanoid failed, trying uuid", {
        error: nanoidError.message,
      });
  ...
  logger.warning("All ID generators failed, using crypto fallback", {
    error: error.message,
  });
  ...
  logger.warning("Using crypto-based fallback ID generator");
}
