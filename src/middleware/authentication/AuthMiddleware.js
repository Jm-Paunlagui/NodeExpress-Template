...
const { AppError, AUTH_ERRORS, VALIDATION_ERRORS } = require("../../constants/errors");
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
      if (err) {
        ...
        if (isFileDownload) {
          ...
          return res
            .status(statusCode)
            .send(AuthMiddleware._authHtmlError(title, line1, line2));
        }
  ...
  static verifyToken(token) {
    return new Promise((resolve, reject) => {
      jwt.verify(token, process.env.JWT_SECRET, (err, payload) => {
        if (err) return reject(new AppError(AUTH_ERRORS.FORBIDDEN_ACCESS, 403));
        resolve(payload);
      });
    });
  }
  ...
  static requireAccess(predicate, options = {}) {
    return (req, res, next) => {
      ...
      if (!predicate(req.user)) {
        return next(
          new AppError(options.message || AUTH_ERRORS.FORBIDDEN_ACCESS, 403, {
  ...
  static validateRequiredFields(requiredFields) {
    return (req, res, next) => {
      ...
      const missing = requiredFields.filter(
        (f) =>
          source[f] === undefined || source[f] === null || source[f] === "",
      );

      if (missing.length > 0) {
        return next(
          new AppError(VALIDATION_ERRORS.MISSING_FIELDS, 400, {
